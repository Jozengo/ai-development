...
class VectorMemoryService(MemoryService):
    ...
    def __init__(self, collection_name: str, base_dir: Optional[str] = None):
        """初始化向量记忆服务
        ...
        self.logger.info(f"初始化向量记忆服务，集合名称: {collection_name}")
...
class PrivateMemoryService(VectorMemoryService):
    ...
    def __init__(self, user_id: str, base_dir: Optional[str] = None):
        ...
        Args:
            ...
            base_dir: 基础存储目录，如果为None则使用默认目录
        """
        collection_name = f"private_{user_id}"
        super().__init__(collection_name, base_dir)
        self.user_id = user_id
        self.logger.info(f"初始化用户私有记忆服务，用户ID: {user_id}")
...
class PublicMemoryService(VectorMemoryService):
    ...
    def __init__(self, base_dir: Optional[str] = None):
        ...
        Args:
            base_dir: 基础存储目录，如果为None则使用默认目录
        """
        super().__init__("public_memory", base_dir)
        self.logger.info("初始化公共记忆服务")
...
class MemoryServiceFactory:
    ...
    def __init__(self, base_dir: Optional[str] = None):
        ...
        self._private_services: Dict[str, PrivateMemoryService] = {}
        self._public_service: Optional[PublicMemoryService] = None
    ...
    def get_private_memory_service(self, user_id: str) -> PrivateMemoryService:
        ...
        if user_id not in self._private_services:
            self._private_services[user_id] = PrivateMemoryService(user_id, self.base_dir)
        return self._private_services[user_id]
    ...
    def get_public_memory_service(self) -> PublicMemoryService:
        ...
        Returns:
            公共记忆服务实例
        """
        if self._public_service is None:
            self._public_service = PublicMemoryService(self.base_dir)
    ...
    async def close_all(self) -> None:
        ...
        # 关闭私有记忆服务
        for service in self._private_services.values():
            await service.close()

        # 关闭公共记忆服务
        if self._public_service:
