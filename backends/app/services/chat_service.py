...
from backends.app.services import agent_tools as tools
from backends.app.services.memory_service import MemoryServiceFactory

from backends.app.core.llms import model_client
from backends.app.schemas.chat import ChatMessage
...
class ChatService:
    ...
    def __init__(self):
        ...
        self.tools = [tools.track_order, tools.submit_feedback, tools.request_human_support, tools.get_product_details,
                      tools.get_current_promotions, tools.create_return_exchange]
        # 默认系统提示
    ...
    async def chat_stream(self, messages: List[ChatMessage],
        ...
        try:
            ...
            agent = AssistantAgent(
                ...
                memory=[session.chat_memory_service.memory,
                        session.public_memory_service.memory,
                        session.private_memory_service.memory]  # 使用包含上下文的记忆列表
            )
            ...
            async for event in agent.run_stream(task=last_user_message):
                if isinstance(event, ModelClientStreamingChunkEvent):
                    ...
                    # 用户提问
                    await session.chat_memory_service.memory.add(MemoryContent(content=event.messages[0].model_dump_json(), mime_type=MemoryMimeType.JSON))
                    # AI回答
                    await session.chat_memory_service.memory.add(MemoryContent(content=event.messages[-1].model_dump_json(), mime_type=MemoryMimeType.JSON))
            # 保存聊天历史
